...
<The two or three ideas this tutorial rests on, for an engineer who has not built a
task runner before. Say what the idea is, why runners need it, and what goes wrong
without it. Lists, not long sentences. One analogy is allowed. The block must cover
four things, one bullet or more per thing: (1) what you do in this chapter, in one
line; (2) why this shape, with one line on how the fleet orchestrator in
/Users/sergii/git/fleet handles it, naming the part that differs in an interesting
way rather than every part each time; (3) how this moves on from the previous
tutorial, naming its number and what it could not do; (4) the tricky cases, the one
or two traps the code guards against. Anything already said in detail later (`How
it works`, `Design decisions`, `Under the hood`) is previewed here in one line, not
repeated in full.>
...
src/swarm/
  queue/
+   beads.py          <one plain phrase>
~   runner.py         <what changed, one phrase>
tests/
+   test_beads.py
```
...
<The mechanism, in prose. Walk one task through the new code: where it waits, which
worker picks it, what happens on success and on failure. A short text diagram of the flow
is welcome. Name files in prose; do not quote them here.>

### Design decisions

<Two or three, each about runner behaviour, each two sentences: the choice, and what it
protects. Where the fleet orchestrator in /Users/sergii/git/fleet chose differently for
scale, say so.>
...
<Optional. One fact that surprises, such as a queue quirk the code works around.>
...
Audience and depth
- You are an engineer who knows Python, git, and environment files. The tutorial
  never explains those. It explains runner and queue ideas, the why behind a design
  choice, and anything a worker does that a programmer would not expect.
...
- Runner-focused, not pythonic. Every paragraph is about what the task does, where it
  waits, which worker picks it, or what the runner enforces. A dataclass versus a
  library, a constant versus a literal, how a function is laid out: none of that
  belongs in a tutorial. If a sentence would read the same in a Django tutorial,
  delete it.
...
- Plain English, short sentences, one idea per sentence. Paragraphs, not one sentence
  per line.
- Spell out a domain abbreviation the first time it appears (LLM, CLI, DAG). Everyday
...
- Links between tutorials are relative file names such as `tut03-questions.md`.
